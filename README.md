...
```
 ...
 Airflow ── гоняет ETL и стриминг-микробатчи (DAG'и ниже)
```
...
В `dags/` пять DAG'ов — четыре про данные, один про сам Airflow:
...
- **Версия коннектора Spark↔Kafka = версии Spark.** `spark-sql-kafka-0-10_2.12:3.5.5` соответствует Spark 3.5.5 из образа. Обновишь образ — поменяй версию в `jobs/kafka_to_iceberg.py`, `dags/kafka_pipeline.py` и ноутбуке 07.
...
├── jobs/                   # gp_to_iceberg.py (ETL), kafka_to_iceberg.py (стриминг)
├── dags/                   # stack_healthcheck, gp_to_iceberg_etl, e2e_pipeline_test, kafka_pipeline, airflow_patterns
